...
meta_raw1 <- "raw_meta/DoubleSeqBatch1_20220531.csv"
meta_raw2 <- "raw_meta/DoubleSeqBatch2_20220531.csv"
meta_raw3 <- "raw_meta/DoubleSeqBatch3_20220531.csv"

tidy_raw_metadata <- function(meta_file, batch){ 
  if (batch == 1 | batch == 2){
    comb_col_names <- c("Study_Participant_ID", "Embryo_ID", "AOD", "GC", "Oocyte_Age",
                      ...
                      "Num_MII", "Num_2PN", "Num_Blasts", "Embryoscope", "TMC", "Time_to_OPU_hr",
    ...
    comb_col_names <- c("Study_Participant_ID", "Embryo_ID", "AOD", "GC", "Oocyte_Age",
                      ...
                      "Past_surgical_hist", "FSH_D3", "AMH", "AFC", "LH", "E2_Day_2_3",
                      "Type_of_trigger", "Total_Gn", "E2_on_trigger", "Num_eggs",
                      "Num_MII", "Num_2PN", "Num_Blasts", "Embryoscope", "TMC",
                      "Embryo_grade_at_freezing", "Interpretation", "Natural_cycle",
                      "Medication", "lining_thickness_mm", "Prednisone", "Fragmin",
                      "EG", "HCG", "IL", "GCSF", "Metformin","PRP","Sildenafil",
                      "Transferring_physician", "Transfer_catheter_used", "Pregnant", "Ongoing_pregnancy",
  ...
  meta <- read.csv(meta_file, header = FALSE, skip=1, col.names = comb_col_names, row.names = 2) %>% 
    ...
    mutate(Interpretation = recode(Interpretation, "EUPLOID"=0, "MOSAIC"=1, "MOSIAC"=1)) %>%
    ...
    mutate(cDNA_RT_Date = recode(cDNA_RT_Date,  "19-Mar-20"="1", "13-Apr-20"="2", "17-Jul-20"="3", "12-Aug-20"="4", "18-Aug-20"="5", "25-Aug-20"="6", "2-Sep-20"="7", "8-Sep-20"="8", "23-Sep-20"="9", "16-Oct-20"="10", "27-Oct-20"="11", "5-Nov-20"="12", "9-Dec-20"="13", "14-Jan-21"="14", "22-Feb-21"="15", "26-Mar-21"="16", "5-May-21"="17", "27-May-21"="18", "22-Jun-21"="19", "20-Jul-21"="20", "2-Sep-21"="21", "28-Sep-21" = "22", "4-Nov-21"="23", "22-Jan-22"="24")) %>%
    mutate(Library_Prep_Date = recode(Library_Prep_Date, "11-Dec-20"="0", "11-Feb-21"="1", "12-Aug-21"="2", "1-Mar-22"="3")) %>%
    mutate(Sequencing_Date = recode(Sequencing_Date, "14-Dec-20"= "0", "5-Mar-21"="1", "28-Sep-21"="2", "15-Mar-22"="3")) %>%
  ...
  meta <- separate(meta, col="NInfD", into=c("InfD_SSM_GC", "InfD_Egg_factor", "InfD_MF", "InfD_Uterine_factor", "InfD_TF", "InfD_RPL", "InfD_RIF", "InfD_Unexplained"), sep='_') %>% 
    ...
    within(rm("Infertility_diagnosis", "Past_med_hist")) %>% as.data.frame()
  
  return(meta)

}

meta_tidied1 <- tidy_raw_metadata(meta_raw1, 1)
write.csv(meta_tidied1, "tidied_meta/tidied_meta_batch1_CREATE_kw.csv")
meta_tidied2 <- tidy_raw_metadata(meta_raw2, 2)
write.csv(meta_tidied2, "tidied_meta/tidied_meta_batch2_CREATE_kw.csv")
meta_tidied3 <- tidy_raw_metadata(meta_raw3, 3)
write.csv(meta_tidied3, "tidied_meta/tidied_meta_batch3_CREATE_kw.csv")

#meta_full <- rbind(meta_tidied1,  meta_tidied2)
meta_full <- data.table::rbindlist(list(meta_tidied1, meta_tidied2, meta_tidied3), fill=TRUE) %>% `rownames<-`(c(rownames(meta_tidied1), rownames(meta_tidied2), rownames(meta_tidied3)))
...
write.csv(meta_full, "tidied_meta/tidied_meta_CREATE_kw_20220531.csv")

naniar::vis_miss(meta_full)
