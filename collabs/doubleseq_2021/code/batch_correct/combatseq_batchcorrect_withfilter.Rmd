...
ggplot(data_long_logged_cf, aes(x=value, col=name)) + geom_density(alpha=0.2) + theme_bw() + theme(legend.position = "none", panel.background = element_blank(), panel.grid = element_blank()) + xlab("log2(adj count + 1)")
```

```{r colorbyoutcome_corrected}
ggplot(with_batch_lcf, aes(x=value, group_by = name, col=as.factor(Pregnant))) + geom_density(alpha=0.2) + theme_bw() + theme(panel.background = element_blank(), panel.grid = element_blank()) + xlab("log2(adj count + 1)")
```

```{r facetbycdna_corrected}
ggplot(with_batch_lcf, aes(x=value, group_by = name, col=as.factor(Pregnant))) + geom_density(alpha=0.2) + theme_bw() + theme(panel.background = element_blank(), panel.grid = element_blank()) + facet_wrap(~cDNA_RT_Date) + xlab("log2(adj count + 1)")
```

```{r facetbyseq_corrected}
ggplot(with_batch_lcf, aes(x=value, group_by = name, col=as.factor(Pregnant))) + geom_density(alpha=0.2) + theme_bw() + theme(panel.background = element_blank(), panel.grid = element_blank()) + facet_wrap(~Sequencing_Date) + xlab("log2(adj count + 1)")
```
...
counts_lib_norm <- sweep(counts_filter, 2, colSums(counts_filter), `/`)
averaged_batches <- cbind(sapply(1:19, function(x) if (length(which(meta$cDNA_RT_Date == x)) > 1) rowMeans(counts_lib_norm[,which(meta$cDNA_RT_Date == x)]) else counts_lib_norm[,which(meta$cDNA_RT_Date == x)]))
batchDists <- dist(t(averaged_batches))
batchDistMatrix <- as.matrix(batchDists)
rownames(batchDistMatrix) <- paste0("batch", 1:19)
colnames(batchDistMatrix) <- paste0("batch", 1:19)
pheatmap(batchDistMatrix, clustering_distance_rows=batchDists, angle_col = 45)
```
