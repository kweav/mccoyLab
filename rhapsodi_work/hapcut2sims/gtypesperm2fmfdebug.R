...
writefmf<-function(fmf,outfile){
  ...
  sublists_per_list <- unlist(lapply(1:nrow(fmf), function(x) length(fmf[x]$V1[[1]])))
  number_sublists <- unique(sublists_per_list)
  
  #Let's setup a check to see if all the lists have sublists
  if (!(1 %in% number_sublists)) { #normal way where we expect every row to have a list of sublists in V1
    # Get flat list with fragment names to output
    towrite<-unlist(sapply(1:nrow(fmf),function(x) reformatone(fmf[x,])),recursive=F)

  } else if ((1 %in% number_sublists) & (length(number_sublists) == 1)){ #no sublists for any of the gamete lists in V1
    
    # Get flat list with fragment names to output
    towrite<-sapply(1:nrow(fmf),function(x) reformatone(fmf[x,]))
  
  } else { ##What to do with the (possible?) edge case where some lists have sublists and others don't
    no_sublists = which(sublists_per_list == 1) #let's write a which statement to get the rows with no sublists and the rows with sublists
    sublists = setdiff(1:nrow(fmf), no_sublists)
    
    towrite <- c(sapply(1:length(no_sublists), function(x) reformatone(fmf[no_sublists][x,])),
                 unlist(sapply(1:length(sublists), function(x) reformatone(fmf[sublists][x,])),recursive=F))
    
  }
  
  # write out
  #writeLines(sapply(towrite, paste, collapse="\t"),outfile,sep="\n")
  writeLines(sapply(towrite, paste, collapse=" "), outfile, sep="\n") #hapcut2 requires space delimited not tab delimited
}
...
tfile <- args[1] #"~/data_rmccoy22/sperm_seq_rhapsodi/genModel_output/changing_model_params/g3_s100000_c0.1_se0.001_r0.6_mse0.001_mr0.6/g3_s100000_c0.1_se0.001_r0.6_rs357_variantinfo.vcf"
dacfile <- args[2] #"~/data_rmccoy22/sperm_seq_rhapsodi/genModel_output/changing_model_params/g3_s100000_c0.1_se0.001_r0.6_mse0.001_mr0.6/g3_s100000_c0.1_se0.001_r0.6_rs357_gtypesperm.txt"
bcfile <- args[3] #"~/data_rmccoy22/sperm_seq_rhapsodi/genModel_output/changing_model_params/g3_s100000_c0.1_se0.001_r0.6_mse0.001_mr0.6/g3_s100000_c0.1_se0.001_r0.6_rs357_cellbarcodes.txt"
outstem <- args[4] #"~/data_rmccoy22/sperm_seq_rhapsodi/genModel_output/changing_model_params/g3_s100000_c0.1_se0.001_r0.6_mse0.001_mr0.6/g3_s100000_c0.1_se0.001_r0.6_rs357"
